        ...
        public void Initialize()
        {
            ...
            */
            ...
            if (Utility.IsAprilFool()) {
                commands.Add(this._commandFactory.Create().Setup("!bsr", "!request", "!add", "!srm", "!sr", "点歌", "っ'-')╮").Action(this.Bot.ProcessSongRequest).Help(FlagParameter.Everyone, "usage: %alias%<songname> or <song id>, omit <,>'s. %|%This adds a song to the request queue. Try and be a little specific. You can look up songs on %beatsaver%", s_atleast1));
            }
            else {
                commands.Add(this._commandFactory.Create().Setup("!bsr", "!request", "!add", "!srm", "!sr", "点歌").Action(this.Bot.ProcessSongRequest).Help(FlagParameter.Everyone, "usage: %alias%<songname> or <song id>, omit <,>'s. %|%This adds a song to the request queue. Try and be a little specific. You can look up songs on %beatsaver%", s_atleast1));
            }
            commands.Add(this._commandFactory.Create().Setup("!bsr", "!request", "!add", "!srm", "!sr", "点歌").Action(this.Bot.ProcessSongRequest).Help(FlagParameter.Everyone, "usage: %alias%<songname> or <song id>, omit <,>'s. %|%This adds a song to the request queue. Try and be a little specific. You can look up songs on %beatsaver%", s_atleast1));
